...
public class DesUtil {
    private static String firstKey = "%E8%3D%B0";
    private static String secondKey = "E6%82%B1";
    private static String thirdKey = "%E5%B2%BE";
    ...
    // PublicUtil.parseDate(PublicUtil.getCurrentTime(PublicUtil.DATA_FORMAT),
    // PublicUtil.DATA_FORMAT).getTime();
    // StringBuffer sb = new StringBuffer(String.valueOf(time));
    // for (int i = 0; i < 4; i++) {
    // char c = 'a';
    // c = (char) (c + (int) (Math.random() * 26));
    // sb.insert((int) (Math.random() * (1 + 4)), c);
    // }
    // return strEnc(sb.toString());
    // }

    // /**
    // * 判断请求sessionKey的间隔时间是否在范围内 有误差 50毫秒内算正常
    // *
    // * @return
    // */
    // public static boolean checkSessionKey(String sessionKey, Long checkTime)
    // {
    // if (checkTime == null)
    ...
    // PublicUtil.parseDate(PublicUtil.getCurrentTime(PublicUtil.DATA_FORMAT),
    // PublicUtil.DATA_FORMAT).getTime();
    // if (sessionKey == null || checkTime < 0)
    // return false;
    // sessionKey = strDec(sessionKey);
    ...
    // }
    // }
    // long sessionTime = Long.parseLong(sb.toString());
    // long maxTime = SystemConfig.getLong(Constant.SYSTEM_TOKEN_MAX_TIME, 200);
    // // System.out.println(checkTime +"------->"+ sessionTime +"------->"+
    // (checkTime - sessionTime) +"------->"+ maxTime);
    // return checkTime > sessionTime && checkTime - sessionTime > (maxTime > 0
    // ? maxTime : 200);
    // }
